...
*   **Dự đoán thời gian thực (Real-time Inference):** Tích hợp Amazon API Gateway, AWS Lambda và AWS SageMaker Serverless Endpoint để xử lý request và trả về xác suất rời bỏ dịch vụ tức thì với chi phí tối ưu (0đ khi không có lưu lượng).
*   **Kích hoạt tự động (Event-Driven Trigger):** Tự động phát hiện khi Admin tải dữ liệu mới lên Amazon S3, kiểm tra Data Drift và khởi chạy luồng Retrain.
*   **Luồng làm việc MLOps (SageMaker Pipeline - 4 bước):**
...
*   **Triển khai tự động (Continuous Deployment - CD):** Sử dụng Amazon EventBridge lắng nghe trạng thái Approved từ Model Registry để kích hoạt AWS Lambda Deployer tự động cập nhật Serverless Endpoint mà không gây gián đoạn dịch vụ (Zero-Downtime Deployment).
*   **Giám sát & Báo động (Monitoring & Alerting):** Lưu trữ log tập trung qua CloudWatch Logs, thiết lập CloudWatch Alarm và gửi email cảnh báo tự động về hòm thư qua Amazon SNS.

#### Sơ đồ Kiến trúc Hệ thống (Architecture Diagram)

![System Architecture Diagram](/images/5-Workshop/5.1-Workshop-overview/diagram.jpg)
